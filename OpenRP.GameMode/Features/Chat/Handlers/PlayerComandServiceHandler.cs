...
namespace OpenRP.GameMode.Features.Chat.Handlers
{
    /// <summary>Provides player commands functionality.</summary>
    public class PlayerCommandServiceHandler : CommandServiceBase, IPlayerCommandService
    {
        private readonly IEntityManager _entityManager;

        /// <inheritdoc />
        public PlayerCommandServiceHandler(IEntityManager entityManager) : base(entityManager, 1)
        {
        ...
        private static string CommandText(CommandInfo command)
        {
            ...
            {
                return ChatHelper.ReturnPlayerInfoMessage(Enums.PlayerInfoMessageType.SYNTAX, $"/{command.Name}");
            }

            return ChatHelper.ReturnPlayerInfoMessage(Enums.PlayerInfoMessageType.SYNTAX, $"/{command.Name} " + string.Join(" ", command.Parameters.Select(arg => arg.IsRequired
                ? $"[{arg.Name}]"
                : $"<{arg.Name}>")));
        }
